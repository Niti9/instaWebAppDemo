...
export const instagramAuthCallback = async (req, res, next) => {
    ...
    try {
        ...
        if (existingAccount) {
            return res.status(400).json({ message: 'This Instagram account is already linked' });
        }
...
export const getMessages = async (req, res, next) => {
    res.status(501).json({ message: 'Instagram messages integration is not supported via the API' });
};
